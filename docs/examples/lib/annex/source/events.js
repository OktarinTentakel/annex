...
function createSyntheticEvent(
	...
	if( isFunction(EventConstructor) ){
		...
		e = hasValue(payload)
			? new window.CustomEvent(event, {detail : payload, bubbles, cancelable, ...eventOptions})
			// we could use new Event() here, but jsdom and ava cannot use that constructor for dispatchEvent :(
			: new window.CustomEvent(event, {bubbles, cancelable, ...eventOptions})
		;
	}
 ...
 * @param {?Function} [handler=null] - a specific callback function to remove
 * @param {?Boolean} [tryNativeRemoval=false] - if a target is not part of the EVENT_MAP native removeEventListener is used as a fallback if this is true (handler needs to be set in that case)
 ...
 * offSwipe(slider);
 */
export function offSwipe(targets, direction=null, handler=null, tryNativeRemoval=false, eventNameSpace='annex-swipe'){
	...
	directions.forEach(direction => {
		...
		if( hasValue(handler) ){
			...
			if( hasValue(swipeHandler) ){
				removedCount += off(targets, events, swipeHandler, tryNativeRemoval);
			}
		} else {
			removedCount += off(targets, events, null, tryNativeRemoval);
		}
	});
