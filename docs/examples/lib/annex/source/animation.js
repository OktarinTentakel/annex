...
/**
 * @typedef {Object} TypeDef_ClassChanges
 *
 * @property {?string|Array.<string>} [add] - CSS class names to add to the element,
 *   separated by whitespace or as an array
 * @property {?string|Array.<string>} [remove] - CSS class names to remove from the element,
 *   separated by whitespace or as an array
 *
 * @memberof Animation
 */
 ...
 *
 * @param {HTMLElement} element - the element to transition, by applying class and/or style changes
 * @param {?TypeDef_ClassChanges} [classChanges=null] - plain object containing class changes to apply,
 ...
 * @param {?Object.<string,string|number>} [styleChanges=null] - plain object containing style changes to apply
 *   (via applyStyles)
 ...
 * @returns {Basic.Deferred<HTMLElement>} resolves on transition completion or repeated call on the same element,
 ...
 * @throws {Error} if "element" is not an Element,
 *   "classChanges"/"styleChanges" are not PlainObjects
 *   or if transition gets interrupted and "rejectOnInterruption" is true
 *
 ...
 * }).then(() => { console.log('finished'); });
 *
 * transition(element, {add : 'foobar'}, null, true)
 *   .then(() => { console.log('finished'); })
